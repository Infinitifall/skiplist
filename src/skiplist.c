...
#define CL_STARTER_HEIGHT 10


int generate_height() {
    int height = 1;
    while (rand() % 2 == 0) { height += 1; }
    return height;
}
...
Node* create_node(int data) {
    ...
    n->succ_array = malloc(n->height * sizeof(Node*));
    n->pred_array = malloc(n->height * sizeof(Node*));
    return n;
}
...
SkipList* create_list() {
    ...
    l->head_array = malloc(CL_STARTER_HEIGHT * sizeof(Node*));
...
void list_print_pretty(SkipList *l) {
    ...
    while(curr != NULL) {
        ...
        for (int i = 0; i < curr->height; i++) { printf("_"); }
        printf("\n");
        curr = curr->succ_array[0];
    }
}
...
Node* list_search(SkipList *l, int data) {
    ...
    while(curr_height >= 0) {
        ...
        if (next == NULL) { curr_height -=1; }
        else if (next->data > data) { curr_height -=1; }
        else if (next->data == data) { return next; }
        else { break; }
    }
...
void list_insert(SkipList *l, int data) {
    ...
    // double size of head_array if necessary
    while (l->max_height < n->height) {
        l->head_array = realloc(l->head_array, 2 * l->max_height * sizeof(*l->head_array));
        l->max_height *= 2;
    }
    ...
    while(curr_height >= 0) {
        Node *next = l->head_array[curr_height];
        if (next == NULL) {
            if (n->height - 1 >= curr_height) { l->head_array[curr_height] = n; }
            curr_height -=1;
